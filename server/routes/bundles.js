...
const { getBaseUrl, curlPipeCommand } = require('../shell');

function bundleWithItems(userId, bundleId) {
  const bundle = db.prepare('SELECT * FROM bundles WHERE id = ? AND user_id = ?').get(bundleId, userId);
...
router.get('/:id', (req, res) => {
  ...
  if (req.query.format === 'text') {
    ...
    const out = [
      ...
      `安装: ash pull bundle:${bundle.slug}    (${curlPipeCommand(`${getBaseUrl(req)}/s/bundle/${bundle.slug}/install.sh`)})`,
    ];
...
router.post('/', (req, res) => {
  ...
  let finalSlug = slug;
  let n = 2;
  while (db.prepare('SELECT 1 FROM bundles WHERE user_id = ? AND slug = ?').get(req.user.id, finalSlug)) finalSlug = `${slug}-${n++}`;
  const r = db.prepare('INSERT INTO bundles (user_id, slug, name, description) VALUES (?, ?, ?, ?)').run(req.user.id, finalSlug, name.trim(), description);
  res.status(201).json(bundleWithItems(req.user.id, r.lastInsertRowid));
});

// 重命名/改描述
router.put('/:id', (req, res) => {
  const { name, description } = req.body;
  const bundle = db.prepare('SELECT * FROM bundles WHERE id = ? AND user_id = ?').get(req.params.id, req.user.id);
  ...
  res.json(bundleWithItems(req.user.id, bundle.id));
});

// 删除组合（仅删组合与快捷方式，不动技能）
router.delete('/:id', (req, res) => {
  const r = db.prepare('DELETE FROM bundles WHERE id = ? AND user_id = ?').run(req.params.id, req.user.id);
...
router.post('/:id/skills', (req, res) => {
  ...
  const bundle = db.prepare('SELECT * FROM bundles WHERE id = ? AND user_id = ?').get(req.params.id, req.user.id);
  const skill = db.prepare('SELECT id, is_deleted FROM skills WHERE id = ? AND user_id = ?').get(skill_id, req.user.id);
  if (!bundle) return res.status(404).json({ error: 'Bundle not found' });
...
router.delete('/:id/skills/:skillId', (req, res) => {
  const bundle = db.prepare('SELECT id FROM bundles WHERE id = ? AND user_id = ?').get(req.params.id, req.user.id);
  if (!bundle) return res.status(404).json({ error: 'Bundle not found' });
  db.prepare('DELETE FROM bundle_items WHERE bundle_id = ? AND skill_id = ?').run(bundle.id, req.params.skillId);
  res.json(bundleWithItems(req.user.id, bundle.id));
});
