...
export default function App() {
  ...
  const handleBatchStar = async (explicitIds) => {
    const ids0 = explicitIds && explicitIds.length ? explicitIds : [...selectedIds];
  ...
  const focusSearch = () => {
    ...
    // 列表已展开时同步聚焦：rAF 在后台标签页里会被挂起，下一个按键就漏进全局快捷键
    if (!listCollapsed) {
      focusNow();
      return;
    }
  ...
  const handleDropOnFolder = (event, target) => {
    ...
    if (!ids.length) {
      ...
      if (rawId) ids = [Number(rawId)];
    }
    if (!ids.length) return;
    if (target === 'starred') { handleBatchStar(ids); return; }
    if (target === 'trash') { handleBatchTrash(ids); return; }
    if (typeof target === 'string' && target.startsWith('bundle:')) {
      handleAddSkillsToBundle(Number(target.slice('bundle:'.length)), ids);
      return;
    }
    if (ids.length === 1) { handleMoveSkill(ids[0], target); return; }
    handleBatchMove(target, ids);
  };
  ...
  return (
    ...
    >
      ...
      <AddToBundleModal
        ...
        skillIds={addToBundleTarget?.ids || []}
        label={addToBundleTarget?.label}
        bundles={bundles}
        onClose={() => setAddToBundleTarget(null)}
        onDone={() => { fetchFoldersAndStats(); showToast(`已加入组合（${addToBundleTarget?.ids.length ?? 1} 个技能）`); }}
      />
      <NewBundleModal
